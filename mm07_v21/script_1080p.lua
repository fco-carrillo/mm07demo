--[[
  script.lua -- Basemark ES 2.0 main configuration and control script
  ...
  creating custom Basemark runs with different combinations of tests and settings as well as looping
  for demo and stress testing purposes.

  Conventions:
    - Basemark subsystems are wrapped in Lua objects and are called with convention: subsystem_name.function_name
    - C functions that do not belong to any Basemark subsystem wrapped in Lua object are prefixed with c_ 
    - Most lua functions and variables are placed in tables.

  LUA tables in this script:
    - Config                    Most common Basemark configuration variables
...
config = 
{
    ...
    app_res_stack_size =  8 * 1024 * 1024;
    ...
    allow_seek         =    0; -- for benchmarking this should be 0
    camera_speed       = 20.0;
...
app =
{
    ...
    enter_game_test = function()
        ...
        local sshot = 0 --take screenshots

        app.load_fms(app.game_test.fms_file)
        app.preprocess_cuts()
        app.cut_index = 0

		    if(app.phase == menu.entries[7].run) then
					sshot = 1
				end
...
menu =
{
    ...
    entries =
    {
        ...
        {
            title = "DBG Game test: Hover";
            enter = function()
                app.phase = menu.entries[8].run
            end;
            run = function()
                app.game_test_index = 3
                app.enter_game_test()
