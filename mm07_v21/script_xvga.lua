...
config = 
{
    ...
    io_res_stack_size  = 12 * 1024 * 1024;

    featuretest_res_stack_size = 2 * 1024 * 1024;
    ...
    high_quality       = false;  -- set to true to enable high quality post processing
 
    no_progress        = false;  -- set to true to disable progress bar
    capture            = 0;      -- set to the game test you want to capture
    capture_cut        = 0;      -- set to the cut you want to capture
    capture_frame      = 0;      -- set to the frame you want to capture
...
app =
{
    ...
    create = function()
        ...
        app.coroutine = coroutine.create(
            function()
                while(app.running)
                do
					if(config.capture ~= 0) then             --VIVANTE
					    app.game_test_index = config.capture --VIVANTE
						app.enter_game_test();               --VIVANTE
						c_show_hud(0)                        --VIVANTE
						c_pause(0)                           --VIVANTE
						app.wait()                           --VIVANTE
						app.running = false                  --VIVANTE
						app.exit()                           --VIVANTE
					else                                     --VIVANTE
    ...
    init = function()
        if(app.xsi == 1) then
			...
			if(config.capture ~= 0) then                          --VIVANTE
                app.load_fms(game_tests[config.capture].fms_file) --VIVANTE
            else                                                  --VIVANTE
    ...
    update_cut = function()
        ...
        -- VIVANTE [ BEGIN ]
        if (config.capture_cut ~= 0) then
            if (config.capture_count == 0) then
                c_end_game_test(app.game_test.title)
                return
            end
			app.cut_index = config.capture_cut
          config.capture_count = config.capture_count - 1
		end -- VIVANTE [ END ]
        ...
        if(app.cut_index < 1 or app.cut_index > # game_tests[app.game_test_index].cuts) then
			...
			-- VIVANTE [ BEGIN ]
			if (config.capture_frame ~= 0) then
				c_update_cut(
					app.cut_index,			-- 1
					c.scene_index,			-- 2
					c.composer_index,		-- 3
					c.camera_index,			-- 4
					config.capture_frame,	-- 5 first frame
					config.capture_frame,	-- 6 last frame
					c[6]					-- 7 speed
				)
              
              config.capture_frame = config.capture_frame + 1
			else -- VIVANTE [ END ]
				c_update_cut(
					app.cut_index,    -- 1
					c.scene_index,    -- 2
...
menu =
{
    ...
    entries =
    {
...

        {
            ...
            run = function()
                app.load_fms("feature_tests.fms")
